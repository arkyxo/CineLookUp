...
export default function SearchBox({ onNavigate, className = '' }) {
  ...
  return (
    <form onSubmit={submitFullSearch} className={`relative ${className}`}>
      <SearchIcon size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-ink/40" />
      <input
        value={query}
        ...
        className="w-full rounded-full border border-ink/10 bg-ink/5 py-2 pl-9 pr-3 text-sm outline-none placeholder:text-ink/30 focus:border-crimson-500"
      />

      {open && suggestions.length > 0 && (
        <div className="absolute left-0 right-0 top-full z-50 mt-2 max-h-80 overflow-y-auto rounded-lg border border-ink/10 bg-card shadow-2xl">
